...
async function getLiberdusBalance(page) {
  ...
  for (const row of assetRows) {
    ...
    if (name === 'Liberdus') {
      return parseFloat(await row.$eval('.asset-balance', el => el.textContent.trim()));
    }
  }
...
// ─────── Test suite ─────────────────────────────────────────────
test.describe.serial('Liberdus Web App E2E Tests', () => {
  let browser;
  ...
  test.describe('Tests requiring sign-in', () => {
    ...
    test('shoud send LIB to contact with not memo, and check history', async () => {
  ...
  // ── Multi-user scenario using isolated contexts ───────────────
  test('should allow two users to message each other (separate contexts)', async ({browserName}) => {
  ...
  test('should increase user1 wallet balance by toll when messaged by user2', async ({browserName}) => {
    ...
    let expectedBalance = balanceBefore + readTollAfterTax - NETWORK_FEE;

    // UI only shows 2 decimal places, so we truncate for comparison
    const truncate2 = v => Math.trunc(v * 100) / 100;
    expectedBalance = truncate2(expectedBalance);
